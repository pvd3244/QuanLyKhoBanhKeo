...
<body>
	...
	$sql = "SELECT `soLuongXuat`, `donGia` FROM `phieuxuatsanpham` WHERE maPhieu = $maPhieu AND maCTSP = $maCTSP";
	$sp = mysqli_fetch_assoc(mysqli_query($conn,$sql));
	$sqlSP = "SELECT tenSP, `ngaySanXuat`, `hanSD` 
		FROM `chitietsanpham` INNER JOIN sanpham on chitietsanpham.maSP = sanpham.maSP
		WHERE maCTSP = $maCTSP";
	$ttSP = mysqli_fetch_assoc(mysqli_query($conn,$sqlSP));
	$date = date_create($ttSP["ngaySanXuat"]);
	?>
	<div>
		<p align="right">Xin chào, <?php echo $_SESSION["tenNV"] ?> <a href="xulyDX.php">Đăng xuất</a></p>
	</div>
	<h2 align="center">Sửa sản phẩm xuất</h2>
	<form action="xulySuaSPX.php" method="post">
		<table>
			<tr>
				<td>Tên sản phẩm:</td>
				<td><?php echo $ttSP["tenSP"] ?></td>
			</tr>
			<tr>
				<td>Ngày sản xuất:</td>
			...
			<tr>
				<td>Hạn sử dụng:</td>
				<td><?php echo $ttSP["hanSD"] ?></td>
			</tr>
			<tr>
				<td>Số lượng xuất:</td>
				<td><input type="text" value="<?php echo $sp["soLuongXuat"] ?>" name="soLuongXuat"></td>
			</tr>
			<tr>
				<td>Đơn giá:</td>
				<td><input type="text" value="<?php echo $sp["donGia"] ?>" name="donGia"></td>
			</tr>
		</table>
...
<script>
	function QuayLai(){
		location.replace("ChiTietPhieuXuat.php?id=<?php echo $maPhieu ?>")
	}
</script>